...
/*
   ...
   一个回文串，去掉开头字符和结尾字符后，剩下的字符串仍然是回文串。
       ...
       但是，还存在特殊情况，当 i > j 时，此时 s[i..j] 构成不了子串，所以此时 dp[i][j] 等于 false。
           ...
           此时子串有两个字符，假设子串为 "aa" ，如果调用上面的状态转移方程的话，
           因为 dp[4][3] 等于false，所以会直接让 dp[3][4]也等于 false，
           但是我们知道子串 "aa" 是回文串，所以会造成错误，修改状态转移方程，
               当 s[i] == s[j]，并且子串的长度小于等于2的时候，结果为true。
       综合下来，状态转移方程为
   ...
   这里提一下，外层循环表示当前要求第i行的数据，内层循环表示当前要求第j列的数据
...
func longestPalindrome(s string) string {
	...
	for i := n - 1; i >= 0; i-- {
		for j := i + 1; j < n; j++ {
			dp[i][j] = (s[i] == s[j]) && (dp[i+1][j-1] || (j-i+1 <= 2))
			// 如果 dp[i][j] 是回文串，并且长度大于 maxLen，那么记录更长的子串
