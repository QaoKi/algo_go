...
/*
   ...
   这个思路和 1.组合总和4 中爬楼梯升级版的思路类似，都是取所有可能的值作为最后一个硬币，然后求结果。
...
/*
   方法2，动态规划
       ...
       状态转移方程
           和爬楼梯思路一样，假设最后一个硬币的金额为 C，那么 dp[i] = dp[i-C] + 1，而 C 的取值有多个，
               所以可以求出所有可能的 dp[i]，取最小的一个，
           所以，状态转移方程为
               dp[i] = min(dp[i], dp[i-C] + 1)，(coins[0] <= C <= coins[coins.size()-1])
       base case
           1，因为状态转移方程用求的是 min，所以，我们考虑将 dp[i] 的值初始化为很大的值，这样就不会影响 min() 的值。
               考虑值初始化为 amount+1，凑成 amount 金额的硬币数最多只可能等于 amount（全用1元面值），
